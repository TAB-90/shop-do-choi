...
@section('info_keyword', 'Giỏ hàng, Mua sắm, Sản phẩm, Quản lý giỏ hàng')
    ...
    <div class="cart-container">
                        ...
                        <div class="row align-items-center">
                            <div class="col-12 col-md-2">
                                <div class="cart-image">
                                    <img src="{{ Storage::url($cart->product->cover_image) }}" alt="{{ $cart->product->title }}" class="img-fluid rounded">
                                </div>
                            </div>
                            <div class="col-12 col-md-5">
                                <div class="cart-info">
                                    <h5 class="cart-title">
                                        <a href="{{ route('products.show', $cart->product->slug) }}" class="text-decoration-none">{{ $cart->product->title }}</a>
                                    </h5>
                                    <p class="cart-category">
                                        <span class="text-muted">Danh mục:</span> 
                                        <a href="{{ route('products.index', ['category' => $cart->product->category->slug]) }}" class="text-decoration-none">{{ $cart->product->category->name }}</a>
                                    </p>
                                    <p class="cart-price">{{ number_format($cart->product->price, 0, ',', '.') }}đ</p>
                                </div>
                            </div>
                            <div class="col-12 col-md-2">
                                <div class="cart-quantity">
                                    <div class="quantity-control">
                                        ...
                                        <input type="number" min="1" max="{{ $cart->product->stock }}" value="{{ $cart->quantity }}" class="quantity-input" id="quantity-{{ $cart->id }}" data-cart-id="{{ $cart->id }}">
                            ...
                            <div class="col-12 col-md-2">
                                <div class="cart-total text-end">
                                    <p class="item-total" id="total-{{ $cart->id }}">{{ number_format($cart->product->price * $cart->quantity, 0, ',', '.') }}đ</p>
                                </div>
                            </div>
                    ...
                    <div class="col-12 col-md-6 offset-md-6">
                        <div class="summary-card">
                            ...
                            <div class="summary-actions">
                                <a href="{{ route('products.index') }}" class="btn btn-pry">
                                    <i class="fas fa-arrow-left me-2"></i> Tiếp tục mua sắm
                                </a>
                                <a href="#" class="btn btn-pry">
        ...
        @else
            <div class="empty-cart text-center py-5">
                ...
                <a href="{{ route('products.index') }}" class="btn btn-pry mt-3">Tiếp tục mua sắm</a>
            </div>
        @endif
    </div>
@endsection
